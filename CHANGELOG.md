...
### Tests
* **`MonthlyCheckinDeclarationTest` (new)** guards the five things that make the scoping work: the option value is declared before the group; it is an **active** value in the **activity_type** group; nothing else declares either record (so no filename sort can separate them again); the group carries `extends` alongside its `:name` scoping; and the two answers are real Booleans with `vc_will_ask` nullable.
...
* **And the widened walk had to be widened again**, because the first fix mirrored `mgd-php@2` while `info.xml` declares **`mgd-php@1`** — so it was *narrower than what core actually loads*, which is the one thing this guard must never be. v1 recursively scans the whole extension tree. Verified against core's own `CRM_Utils_File::findFiles()`: both now see the same **65** files. The dot-directory exclusion is core's and is load-bearing — this repository can hold a complete second copy of itself under `.claude/worktrees/` (63 further `.mgd.php` files today), and a naive walk would report every one as a duplicate.
* **Ten mutations checked, each goes red:** drop `extends`; make `vc_will_ask` required; scope by numeric value instead of `:name`; move the option value to the end of the array; split it back into its own file; move it to a different option group; deactivate it; declare a duplicate in `ang/`; declare one in `scripts/`; declare one with double quotes. Plus two that must stay **green** and do — a sibling file mentioning the type only in comments, and the `.claude/worktrees/` copy of the whole extension.
