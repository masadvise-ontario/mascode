...
* **Measured on the 2026-09-21 dev clone:** of 481 `Case Coordinator is` rows with `is_active = TRUE`, **299 are ended** — `end_date` in the past, some back to March 2025 — and 31 of those sit on cases that are not closed. A guard written to stop a link outliving its role would have admitted every one.
