...
| `Job_MasCloseStaleServiceRequests.mgd.php` | Job | stale-SR auto-close (v1.1.29) | Daily scheduled Job running `Mascode.closeStaleServiceRequests` with `allEligible` — the only caller of the unattended mode. Closes SRs in Request RCS more than 64 days old with a sent RCS reminder, into No Client Response. `update='unmodified'`, so disabling it in Scheduled Jobs survives deploys. What it closed is in the CiviCRM log, not the Job log. |
